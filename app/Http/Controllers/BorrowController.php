...
class BorrowController extends Controller
{
    ...
    public function update(Request $request, Borrow $borrow) {
        ...
        if ($book->status == 'diajukan' || $request->status == 'dipinjam') {
            $book->status = '1';
            $book->save();
        ...
        return redirect('/dashboard/borrow')->with('success', 'Status berhasil diubah');
    }

    public function destroy(Borrow $borrow) {
        $borrow::destroy($borrow->id);

        return redirect('/dashboard/borrow')->with('success', 'Pinjaman berhasil dihapus');
    }
